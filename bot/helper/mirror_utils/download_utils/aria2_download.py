...
class AriaDownloadHelper(DownloadHelper):
    ...
    def __onDownloadStarted(self, api, gid):
        ...
        if ENABLE_FILESIZE_LIMIT:
          if size / 1024 / 1024 / 1024 > MAX_TORRENT_SIZE:
              ...
              dl.getListener().onDownloadError(f'Your File Size 〘{get_readable_file_size(size)}〙🙂\n Maximum Mirror Allow 〘{MAX_TORRENT_SIZE}GB〙')
    ...
    def __onDownloadStopped(self, api, gid):
        ...
        dl = getDownloadByGid(gid)
        if dl: dl.getListener().onDownloadError('Your Torrent Link is Dead or Seedless')
